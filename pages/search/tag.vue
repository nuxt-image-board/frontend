...
<section class="section">
   <div class="container is-widescreen">
      ...
      <div class="columns is-centered is-multiline is-mobile">
         <div class="column is-12-mobile is-6-touch is-3-desktop" v-for="result in results.imgs" v-bind:key="result.illustID">
...
export default {
  ...
  async asyncData ({ $axios, route }) {
    ...
    return {
      results: data,
      NotificationTitle: data.title + ' ' + data.count + '件',
      SelectedPage: page,
    ...
    async getData () {
      ...
      const sort = (sortNum <= 1) ? 'd'
        : (sortNum <= 3) ? 'c'
          : 'l'
      const params = { sort, order, page }
      const response = await this.$axios.get('/search/all', { params })
